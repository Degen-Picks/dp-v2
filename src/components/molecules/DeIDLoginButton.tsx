...
const getUsernameFromDeID = (deid: DeIDData) => {
  return deid.username 
    || deid.twitterHandle 
    || deid.discordUsername 
    || (deid.wallets?.length 
        ? (`${deid.wallets[0].address.slice(0, 4)}...${deid.wallets[0].address.slice(-4)}`) 
        : null);
}
...
const DeIDLoginButton: FC = () => {
    ...
    {publicKey && (
      <button
        ...
        className={`group ${
          !!wagerUser?.deidData
            ? "bg-transparent border border-white text-white hover:border-[#f14668] hover:bg-[#f14668] hover:text-white"
            : "bg-transparent border border-white text-white hover:bg-greyscale1 hover:text-black"
        } h-[50px] flex justify-center items-center w-auto
          rounded-full px-5 py-2.5 gap-2 transition-colors duration-300 font-figtree-semi`}
        style={{ minWidth: "131px", borderWidth: "2px", borderRadius: "20px", padding: "10px 20px" }}
      >
        <p className={`text-lg leading-6 ${!!wagerUser?.deidData ? "hidden" : "inline"}`}>
          Connect
        </p>
        {wagerUser?.deidData && (
          <>
            <p className="text-lg leading-6 inline hidden group-hover:inline">
              Unlink
            </p>
            <p className="text-lg leading-6 inline group-hover:hidden">
