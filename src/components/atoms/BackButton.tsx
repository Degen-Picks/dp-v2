...
const BackButton: FC<Props> = ({ text, handleClick }) => {
  return (
    <motion.button
      className="flex items-center gap-2 cursor-pointer h-[50px] w-[100px] group"
      onClick={handleClick}
    >
      <svg
        width="15"
        height="13"
        viewBox="0 0 15 13"
        className="fill-purple1 group-hover:fill-purple2"
      ...
      <p className="text-purple1 group-hover:text-purple2 text-lg">{text}</p>
    </motion.button>
  );
};
